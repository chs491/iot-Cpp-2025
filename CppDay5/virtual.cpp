...
//	void func1() override { cout << "D::func1()" << endl; }		// 가상함수를 재정의하는 멤버함수를 나타낸다.
...
//	bptr->func2();		// Base클래스의 func2()는 가상함수로 선언되었지만 오버라이딩(재정의)이 되지 않았다.
...
//	이때 오버라이딩된 파생 클래스의 멤버를 사용하기 위해서는 기초클래스의 멤버 함수를 가상함수로 만들면 된다.
